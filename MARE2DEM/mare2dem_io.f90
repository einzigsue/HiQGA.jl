...
! 
    ...
    do  ! infinite while loop
        ...
        case ('roughness penalty')
            ...
            case ('mgs','MGS')
				lMGS = .true.  
				if ((i .ne. 0 ).and.(i > len_trim(sValue)) ) then
...
!
    ...
    if (lMGS) then
    	write(cnum,'(4a,es12.4)') 'MGS,',beta_mgs 
...
!    endif
    ...
    ! First we need to copy pm into rhoParams:
    ict = 0
    do i=1,nRegions
        do j = 1,nRhoPerRegion  
            if (iFreeParam((i-1)*nRhoPerRegion + j) > 0 ) then
                ict = ict+1
                if ( (nRhoPerRegion<4) ) then
                    logrho  =  transformToBound(pm(ict),lowerBound(ict),upperBound(ict),lBoundMe(ict) )   
                    rhoParams((i-1)*nRhoPerRegion + j) =  10.d0**logrho
                else
                    rhoParams((i-1)*nRhoPerRegion + j) =  transformToBound(pm(ict),lowerBound(ict),upperBound(ict),lBoundMe(ict) )
                    if (j == 1) then
                        rhoParams((i-1)*nRhoPerRegion + j) =  10.d0**rhoParams((i-1)*nRhoPerRegion + j)
                   !  else
                        ! linear:
                        ! rhoParams((i-1)*nRhoPerRegion + j) =  (rhoParams((i-1)*nRhoPerRegion + j))
                        ! log10:
                        ! rhoParams((i-1)*nRhoPerRegion + j) =  (10.d0**rhoParams((i-1)*nRhoPerRegion + j))
                    endif
                    !write(*,'(i6,1x,4(g12.2,1x))') ict,    pm(ict),lowerBound(ict),upperBound(ict), rhoParams((i-1)*nRhoPerRegion + j)
                endif
           endif
        enddo    
    enddo     
...
!   
        ...
        case ('# dc transmitters')
            ...
            write(*,*) ' '
            write(cnum,'(i6)') nTxDC
            call print2col32('# DC Transmitters: ',cnum,6)

            sFmt = '(a1,1x,3(a12,2x))' 
            write(*,sFmt) ' ','Electrode A','Electrode B','Name'
...
!                 case ('emdata_2.2') 
        ...
        case ('# dc receivers')
            ...
            write(cnum,'(i6)') nRxDC
            write(*,*) ' '
            call print2col32(' # DC Receivers: ',cnum,6) 
            
            sFmt = '(a1,1x,3(a12,2x))' 
            write(*,sFmt) ' ','Electrode M','Electrode N','Name'
...
!                 case ('emdata_2.2') 
        ...
        case ('# dc transmitter electrodes')              
            ...
            write(*,*) ' '
            call print2col32(' # DC Transmitter Electrodes: ',cnum,6) 
        ...
        case ('# dc receiver electrodes')                     
            ...
            write(*,*) ' '
            call print2col32(' # DC Receiver Electrodes: ',cnum,6) 
...
!
    ...
    write(cnum,'(i6)') nDC
    call print2col32(' # DC Resistivity Data: ',cnum,6) 
...
!==================================================================================================================================! 
    ...
    real(RealPrec) :: residMT, residCSEM, residDC, resid
